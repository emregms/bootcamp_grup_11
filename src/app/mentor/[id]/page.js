...
import {
  ...
  MessageSquare, Calendar, Award, ArrowRight, Play, Shield
} from 'lucide-react';
import { mentors, lessons, reviews } from '@/data/mock';
import styles from './page.module.css';

export default function MentorProfilePage({ params }) {
  const { id } = use(params);
  const mentor = mentors.find(m => m.id === parseInt(id)) || mentors[0];
  const mentorLessons = lessons.filter(l => l.mentorId === mentor.id);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.headerBg} style={{ background: `linear-gradient(135deg, ${mentor.color}33, ${mentor.color}11)` }}>
        <div className={`container ${styles.headerContent}`}>
          ...
          >
            <div className={styles.avatar} style={{ background: `linear-gradient(135deg, ${mentor.color}, ${mentor.color}88)` }}>
              {mentor.initials}
              {mentor.verified && (
          ...
          >
            <div className={styles.priceCard}>
              ...
              <Link href={`/lesson/${mentorLessons[0]?.id || 1}`} className="btn btn-secondary w-full">
            ...
            <div className={styles.reviewsList}>
              {mentorReviews.length > 0 ? mentorReviews.map(review => (
                <div key={review.id} className={styles.reviewCard}>
                  ...
                  <span className={styles.reviewLesson}>{review.lessonTitle}</span>
                </div>
              )) : (
