...
function StatCounter({ value, label, icon: Icon }: { value: string; label: string; icon: LucideIcon }) {
  const ref = useRef(null);
...
export default function HomePage() {
  const [featuredMentors, setFeaturedMentors] = useState<Mentor[]>(
    (mentors.filter((m) => m.topMentor).slice(0, 4)) as unknown as Mentor[]
  );
  const [categoryList, setCategoryList] = useState<Category[]>(categories as unknown as Category[]);
  const [stats, setStats] = useState<PlatformStats>(platformStats as unknown as PlatformStats);
    ...
    <>
              ...
              <div className={styles.heroCardBody}>
                <div className={styles.matchPair}>
                  <div className={styles.matchUser}>
                    <div className={styles.matchAvatar} style={{ background: 'var(--gradient-hero)' }}>DY</div>
                    <span>Sen</span>
      ...
      <AnimatedSection className={styles.statsSection}>
        <div className={`container ${styles.statsGrid}`}>
          <StatCounter value={`${(stats.totalUsers || 0).toLocaleString('tr-TR')}+`} label="Aktif Kullanıcı" icon={Users} />
          <StatCounter value={`${(stats.totalLessons || 0).toLocaleString('tr-TR')}+`} label="Tamamlanan Ders" icon={BookOpen} />
          <StatCounter value={`${(stats.totalSwaps || 0).toLocaleString('tr-TR')}+`} label="Başarılı Takas" icon={Repeat} />
            ...
            {categoryList.map((cat) => (
              <motion.div key={cat.id} variants={scaleIn}>
                <Link href={`/explore?category=${cat.id}`} className={styles.categoryCard}>
                  ...
                  <div className={styles.categoryArrow} style={{ color: cat.color ?? undefined }}>
                    <ArrowRight size={16} />
                  </div>
                </Link>
              </motion.div>
